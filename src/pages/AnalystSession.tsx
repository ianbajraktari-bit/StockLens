...
export default function AnalystSession() {
  ...
  function handleStepSubmit(text: string) {
    ...
    // Snapshot "before" XP/level the first time anything is awarded this session.
    const xpPre = getTotalXp();
    const lvlPre = getLevelInfo(xpPre).level;
    ...
    if (stepXp) {
      setXpAwarded((x) => {
        if (x === 0) {
          setXpBefore(xpPre);
          setLevelBefore(lvlPre);
        }
        return x + stepXp.awarded;
      });
      setXpAfter(stepXp.totalXp);
      setLevelAfter(stepXp.currentLevel);
    }
  }
    ...
    if (stepIndex < totalSteps - 1) {
      ...
      if (reward.xp) {
        setXpAwarded((x) => {
          ...
          return x + reward.xp!.awarded;
        });
        setXpAfter(reward.xp.totalXp);
        setLevelAfter(reward.xp.currentLevel);
      } else {
        // Ensure after snapshot is populated even if final award was null.
        const totalNow = getTotalXp();
        setXpAfter((a) => (a === 0 ? totalNow : a));
        setLevelAfter((l) => (l === 1 ? getLevelInfo(totalNow).level : l));
      }
          ...
          {phase === 'complete' && (
            ...
            >
              <RewardsPanel
                ...
                subheading={`You just reasoned through ${company.name} (${company.ticker}) end-to-end.`}
                gradeEyebrow="Analyst Mode"
                gradeMessage={`All ${totalSteps} steps finished. Re-read your Thesis — is it specific enough to be wrong?`}
                takeaways={[
                  'Re-read your Thesis step. Is it specific enough to be wrong?',
                  `Check ${company.ticker}'s most recent 10-K on SEC EDGAR. Did you miss any risks?`,
                  'Pick another company. Repeating the workflow is what turns theory into reflex.',
                ]}
                xpAwarded={xpAwarded}
                xpBefore={xpBefore}
                xpAfter={xpAfter}
                ...
                progressCaption={`${company.sector} • ${company.dataAsOf}`}
              />
